						...
						<div className="flex">
							{actorDetails.birthday ? (
								<h4 className="subtitle lemon margin-right">
									Birthday : <span className="bold-normal">{actorDetails.birthday}</span>
								</h4>
							) : null}
							{actorDetails.place_of_birth ? (
								<h4 className="subtitle lemon margin-right">
									Place of birth : <span className="bold-normal">{actorDetails.place_of_birth}</span>
								</h4>
							) : null}
							{actorDetails.deathday ? (
								<h4 className="subtitle lemon margin-right">
									Death day : <span className="bold-normal">{actorDetails.deathday}</span>
								</h4>
							) : null}
						</div>
						<div className="flex">
							{actorDetails.homepage ? (
								<h4 className="subtitle lemon margin-right">
									...
									<span className="bold-normal">
										<a className="font-size-1" href={actorDetails.homepage}>
											{actorDetails.homepage}
										</a>
									</span>
								</h4>
			...
			<div>
				...
				<div className="container-cards">
					{actorMoviesList.map((actorMovie) => (
						<Card media_type="movie" item={actorMovie} key={actorMovie.id + actorMovie.title} />
					))}
				</div>
			</div>
			{actorTvList ? (
				<div>
					...
					<div className="container-cards">
						{actorTvList.map((actorTv) => (
							<Card media_type="tv" item={actorTv} key={actorTv.id + actorTv.name} />
						))}
					</div>
				</div>
			) : null}
